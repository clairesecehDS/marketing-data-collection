#!/bin/bash
#
# Déploiement Cloud Run Job pour verbus-480211 - Scolaires
...
REGION="europe-west1"
JOB_NAME="spyfu-monthly-scolaires"
IMAGE_NAME="gcr.io/${PROJECT_ID}/${JOB_NAME}"

# Configuration SpyFu pour Scolaires
# Utilise config_verbus_scolaires.yaml
CONFIG_FILE="config_verbus_scolaires.yaml"

echo "============================================================"
echo "  Déploiement Cloud Run Job - Verbus Scolaires"
echo "============================================================"
...
gcloud run jobs create $JOB_NAME \
    --image $IMAGE_NAME --region $REGION --project $PROJECT_ID \
    --max-retries 2 --task-timeout 3600 --memory 2Gi --cpu 1 \
    --set-env-vars "SPYFU_CONFIG_FILE=${CONFIG_FILE},GCP_PROJECT_ID=${PROJECT_ID}" \
...
gcloud scheduler jobs create http ${JOB_NAME}-scheduler \
    --location $REGION --schedule "20 0 1 * *" --time-zone "UTC" \
    --uri "https://${REGION}-run.googleapis.com/apis/run.googleapis.com/v1/namespaces/${PROJECT_ID}/jobs/${JOB_NAME}:run" \
    --http-method POST --oauth-service-account-email "${PROJECT_ID}@appspot.gserviceaccount.com" --project $PROJECT_ID \
    || gcloud scheduler jobs update http ${JOB_NAME}-scheduler \
        --location $REGION --schedule "20 0 1 * *" --time-zone "UTC" \
        --uri "https://${REGION}-run.googleapis.com/apis/run.googleapis.com/v1/namespaces/${PROJECT_ID}/jobs/${JOB_NAME}:run" \
        --http-method POST --oauth-service-account-email "${PROJECT_ID}@appspot.gserviceaccount.com" --project $PROJECT_ID

echo "✅ Déploiement terminé! Schedule: 1er de chaque mois à 1h20 (heure de Paris)"
