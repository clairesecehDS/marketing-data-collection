...
REGION="europe-west1"
JOB_NAME="spyfu-monthly-transport"
IMAGE_NAME="gcr.io/${PROJECT_ID}/${JOB_NAME}"
# Configuration SpyFu pour Transport
# Utilise config_verbus_transport.yaml
CONFIG_FILE="config_verbus_transport.yaml"

echo "Déploiement Cloud Run Job - Verbus Transport"
...
gcloud scheduler jobs create http ${JOB_NAME}-scheduler --location $REGION --schedule "30 0 1 * *" --time-zone "UTC" --uri "https://${REGION}-run.googleapis.com/apis/run.googleapis.com/v1/namespaces/${PROJECT_ID}/jobs/${JOB_NAME}:run" --http-method POST --oauth-service-account-email "${PROJECT_ID}@appspot.gserviceaccount.com" --project $PROJECT_ID || gcloud scheduler jobs update http ${JOB_NAME}-scheduler --location $REGION --schedule "30 0 1 * *" --time-zone "UTC" --uri "https://${REGION}-run.googleapis.com/apis/run.googleapis.com/v1/namespaces/${PROJECT_ID}/jobs/${JOB_NAME}:run" --http-method POST --oauth-service-account-email "${PROJECT_ID}@appspot.gserviceaccount.com" --project $PROJECT_ID
echo "✅ Déploiement terminé! Schedule: 1er de chaque mois à 1h30 (heure de Paris)"
