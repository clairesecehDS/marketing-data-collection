#!/bin/bash
#
# Déploiement Cloud Run Job pour International SOS
# Collection SpyFu hebdomadaire
#
...
REGION="europe-west1"
JOB_NAME="spyfu-sos-collection"
IMAGE_NAME="gcr.io/${PROJECT_ID}/${JOB_NAME}"

# Configuration SpyFu pour International SOS
CONFIG_FILE="config_sos.yaml"
...
gcloud builds submit \
    --config=cloud_job_sos/cloudbuild.yaml \
...
# 5. Créer le Cloud Scheduler (tous les lundis à 1h UTC = heure de Paris)
echo "⏰ Configuration du Cloud Scheduler..."
gcloud scheduler jobs create http spyfu-sos-scheduler \
    --location $REGION \
    --schedule "0 1 * * 1" \
    ...
    --project $PROJECT_ID \
    || gcloud scheduler jobs update http spyfu-sos-scheduler \
        --location $REGION \
        --schedule "0 1 * * 1" \
        --time-zone "Europe/Paris" \
        --uri "https://${REGION}-run.googleapis.com/apis/run.googleapis.com/v1/namespaces/${PROJECT_ID}/jobs/${JOB_NAME}:run" \
...
echo "☁️  Job: $JOB_NAME"
echo "⏰ Schedule: tous les lundis à 1h du matin (heure de Paris)"
echo ""
