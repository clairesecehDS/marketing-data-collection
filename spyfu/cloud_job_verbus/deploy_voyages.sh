#!/bin/bash
#
# Déploiement Cloud Run Job pour Verbus - Secteur Voyages
...
# 5. Créer le Cloud Scheduler (1er du mois à 6h UTC, décalé pour éviter collision)
echo "⏰ Configuration du Cloud Scheduler..."
gcloud scheduler jobs create http spyfu-verbus-voyages-scheduler \
    --location $REGION \
    --schedule "0 6 1 * *" \
    --time-zone "UTC" \
    ...
    --project $PROJECT_ID \
    || gcloud scheduler jobs update http spyfu-verbus-voyages-scheduler \
        --location $REGION \
        --schedule "0 6 1 * *" \
        --time-zone "UTC" \
        --uri "https://${REGION}-run.googleapis.com/apis/run.googleapis.com/v1/namespaces/${PROJECT_ID}/jobs/${JOB_NAME}:run" \
...
echo "☁️  Job: $JOB_NAME"
echo "⏰ Schedule: 1er de chaque mois à 6h UTC"
echo ""
