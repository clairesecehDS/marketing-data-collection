#!/bin/bash
#
# Déploiement Cloud Run Job pour Verbus - Secteur Voyages Scolaires
...
REGION="europe-west1"
JOB_NAME="spyfu-verbus-scolaires"
...
echo "============================================================"
echo "  Déploiement Cloud Run Job - Verbus Voyages Scolaires"
echo "============================================================"
...
# 5. Créer le Cloud Scheduler (1er du mois à 2h UTC, décalé pour éviter collision)
echo "⏰ Configuration du Cloud Scheduler..."
gcloud scheduler jobs create http spyfu-verbus-scolaires-scheduler \
    --location $REGION \
    --schedule "0 2 1 * *" \
    --time-zone "UTC" \
    ...
    --project $PROJECT_ID \
    || gcloud scheduler jobs update http spyfu-verbus-scolaires-scheduler \
        --location $REGION \
        --schedule "0 2 1 * *" \
        --time-zone "UTC" \
        --uri "https://${REGION}-run.googleapis.com/apis/run.googleapis.com/v1/namespaces/${PROJECT_ID}/jobs/${JOB_NAME}:run" \
...
echo "☁️  Job: $JOB_NAME"
echo "⏰ Schedule: 1er de chaque mois à 2h UTC"
echo ""
